...
char * 
serializeSchema(Schema *schema) //add
{
  int i;
  VarString *result;
  MAKE_VARSTRING(result);
 ...
 * Description: It will return the schema.
 *
 */
    ...
    for (; string[i] != NULL; i ++ ) {
        if (string[i] == '<') {
            ...
            for(j = 0; string[i] != '>'; j ++, i ++) {
                numberOfAttributes[j] = string[i];
            }
        }
    }
    schema->numAttr = atoi(numberOfAttributes);
            ...
            for (j = 0; j < schema->numAttr; j ++, i ++) {
                ...
                switch (type) {
                    ...
                    case 'S':
                        ...
                        char strLen[10];
                        int strLenCounter = 0;
                        for (; tempString[strCounter] != '['; strCounter ++);
                        strCounter ++;
                        for (; tempString[strCounter] != ']'; strLenCounter ++)
                            strLen[strLenCounter] = tempString[strCounter ++];
                        schema->typeLength[j] = atoi(strLen);
                        break;
                    case 'B':
